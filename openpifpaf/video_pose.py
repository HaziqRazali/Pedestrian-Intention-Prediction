...
def main():
    ...
    vid_w = 960
    vid_h = 378
    vid = cv2.VideoWriter("out_pose.avi", cv2.VideoWriter_fourcc(*'MJPG'), 10, (vid_w,vid_h))
    for data, targets, meta in jaad_val_loader:
    
        print(meta[0]["path_to_scene"][:-4]+".skeleton.png")
        ...
        with show.image_canvas(im, "im_pose.png", show=args.show, fig_width=10.0, dpi_factor=1.0) as ax: skeleton_painter.keypoints(ax, keypoint_sets, scores=scores, colors=colors)
        # write to video
        im = cv2.imread("im_pose.png")
        vid.write(im)
