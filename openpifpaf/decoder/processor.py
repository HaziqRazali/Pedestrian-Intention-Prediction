...
class Processor(object):
    ...
    def fields(self, image_batch):
        ...
        with torch.no_grad():
            heads = self.model(image_batch, head="pifpaf")
            
            #print(len(heads))
            #print(len(heads[0]), len(heads[1])) # 4,5
            #print(heads[0][0].size())
            #print(torch.max(heads[0][0]))
            #sys.exit(0)

            # to numpy
            fields = [[field.cpu().numpy() for field in head] for head in heads]

            #print("max ", np.max(fields[0][0]))
    ...
    def keypoint_sets(self, fields):
        ...
        #print(len(fields))                    # 2
        #print(len(fields[0]))                 # 4
        #print(len(fields[0][0]))              # 17
        #print(fields[0][0].size())
