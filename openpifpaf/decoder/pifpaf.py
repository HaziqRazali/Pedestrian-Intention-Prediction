...
class PifPaf(Decoder):
    ...
    def __call__(self, fields):
        ...
        #print("here ", np.max(pif[0]))       # here  118.24356591701508
        #print("here ", np.max(pif[:,0,:,:])) # here  0.4635017216205597
        #print(np.shape(pif))
        #sys.exit(0)
...
class PifPafGenerator(object):
    ...
    def _target_intensities(self, v_th=0.1, core_only=False):
        ...
        #print("he ", np.shape(self.pif))
        #print(np.max(self.pif[:,0,:,:]))
        #sys.exit(0)
