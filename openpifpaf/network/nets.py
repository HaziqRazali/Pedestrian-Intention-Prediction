...
class Shell(torch.nn.Module):
    ...
    def forward(self, x, head):  # pylint: disable=arguments-differ
        ...
        if head=="pifpaf":
            return [hn(x) for hn in self.head_nets[:2]]
        if head=="crm":
            return [hn(x) for hn in self.head_nets[2:]]
...
def factory_from_scratch(basename, headnames, *, pretrained=True):
    ...
    #if(0):
    #    print("Loading pifpaf pretrained basenet")
    #    pifpaf_pretrained_basenet = models.resnet50(pretrained=True)
    #    pifpaf_pretrained_basenet = torch.nn.Sequential(*(list(pifpaf_pretrained_basenet.children())[:-2])) 
    #    pifpaf_pretrained_basenet.load_state_dict(torch.load("./outputs/pifpaf-resnet50-new.pt")) 
    #    for pair in itertools.zip_longest(base_vision.state_dict().items(), pifpaf_pretrained_basenet.state_dict().items()):
    #            
    #        if(pair[1] is None):
    #            continue
    #           
    #            key_base_vision = pair[0][0]
    #            key_pifpaf_pretrained_basenet = pair[1][0]
    #            print(key_base_vision, key_pifpaf_pretrained_basenet)   
    #            
    #            # print(key_pifpaf, key_resnet)            
    #            # sanity check
    #            if(torch.all(torch.eq(base_vision.state_dict()[key_base_vision], pifpaf_pretrained_basenet.state_dict()[key_pifpaf_pretrained_basenet]))):
    #                print("check if value not supposed to be equal")
    #            
    #            #basenet.state_dict()[key_resnet] = net_cpu.state_dict()[key_pifpaf]
    #            base_vision.state_dict()[key_base_vision].copy_(pifpaf_pretrained_basenet.state_dict()[key_pifpaf_pretrained_basenet])
    #            
    #            # should always be true
    #            if(not torch.all(torch.eq(base_vision.state_dict()[key_base_vision], pifpaf_pretrained_basenet.state_dict()[key_pifpaf_pretrained_basenet]))):
